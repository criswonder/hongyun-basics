...
public class WheelView extends View {
	...
	private int getDesiredWidth(LinearLayout layout) {
		if (layout != null && layout.getChildAt(0) != null) {
			...
			Log.d(TAG, "getDesiredWidth	itemWidth=" + itemWidth);
		}

		int desired = itemWidth * VISIBLEITEMS - itemWidth
				* ITEM_OFFSET_PERCENT *2/ 100;
	...
	private int getItemWidth() {
		...
		Log.d(TAG, "getItemWidth	getWidth() / visibleItems=" + getWidth()
				/ VISIBLEITEMS);
	...
	private int calculateLayoutHeight(int heightSize, int mode) {
		...
		itemsLayout.measure(MeasureSpec.makeMeasureSpec(0,
		...
		itemsLayout.measure(MeasureSpec.makeMeasureSpec(0,
				MeasureSpec.UNSPECIFIED), MeasureSpec.makeMeasureSpec(height
				- 2 * padding, MeasureSpec.EXACTLY));
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		Log.d(TAG, "onMeasure	" + "widthSize=" + widthSize + ",heightSize="
				+ heightSize);
	...
	/**
	 * Builds view for measuring OnMeasure的时候调用
	 */
	private void buildViewForMeasuring() {
		...
		for (int i = currentItem + addItems; i >= currentItem - addItems; i--) {
			...
			if (addViewItemToItemsLayout(i, true)) {
				...
				Log.d(TAG,
						"buildViewForMeasuring	addViewItemToItemsLayout failed");
			}
		}
	...
	private boolean addViewItemToItemsLayout(int index, boolean first) {
		...
		if (view != null) {
			...
			Log.d(TAG, "addViewItemToItemsLayout true index=" + index
					+ ",insert first=" + first);
			return true;
		}
		Log.d(TAG, "addViewItemToItemsLayout false index=" + index
				+ ",insert first=" + first);
	...
	private View getItemView(int index) {
		...
		Log.d(SCR, "getItemView isValidItemIndex(index):"
	...
	private void drawCenterRect(Canvas canvas) {
		...
		// offset);
		// centerDrawable.setBounds(center - offset, 0, center +
		// offset,getHeight());

		float density = getResources().getDisplayMetrics().density;
		float offset = centerDrawable.getIntrinsicWidth() / 2 * density;
		centerDrawable.setBounds(center - (int) offset, 0, center
	...
	private ItemsRange getItemsRange() {
		...
		Log.d(SCR, "getItemsRange	after while loop, first:" + first + ",count:"
				+ count);
	...
	protected void refreshItemLooks() {
		...
		for (int i = 0; i < itemsLayout.getChildCount(); i++) {
			if (i == currentItemInLauout) {
				viewAdapter
						.chageItemAppearance(itemsLayout.getChildAt(i), true);
			} else {
				viewAdapter.chageItemAppearance(itemsLayout.getChildAt(i),
						false);
			}
		}
	}
